  ...
  useEffect(() => {
    ...
    const startLocalStream = async () => {
      try {
        ...
        console.warn(
          'Physical camera/mic unavailable or permission denied, initializing clinical video+audio simulation:',
          err.message
        );
        ...
        canvasStreamIntervalRef.current = setInterval(() => {
          ...
          ctx.fillText(
            user?.name || (isDoctor ? 'Dr. Provider' : 'Patient Feed'),
            320,
            225
          );
    ...
    ws.onmessage = async (event) => {
      try {
        ...
        switch (msg.type) {
          ...
          case 'peer-joined':
            setStatusMessage(
              `${msg.name} joined. Initializing consultation...`
            );
            setConnectionStatus('CONNECTING');
  ...
  const handleEndCall = async () => {
    try {
      ...
      await apiClient(`/consultations/${roomId}/end`, {
        method: 'POST',
        body: JSON.stringify({
          clinical_notes: 'Consultation concluded.',
        }),
      });
    } catch (err) {
