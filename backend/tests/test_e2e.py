...
@pytest.mark.asyncio
async def test_comprehensive_end_to_end():
    print("=" * 65)
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=20.0) as client:
        ...
        print("[TEST 2 PASSED] Default admin login")
        ...
        res_upload = await client.post(
            "/api/admin/upload-photo", files=files, headers=headers_admin
        )
        ...
        assert uploaded_avatar_url.startswith(
            "/uploads/doctors/"
        ), f"Unexpected avatar URL: {uploaded_avatar_url}"
        print(f"Admin uploaded doctor photo successfully: {uploaded_avatar_url}")

        print("[TEST 3 PASSED] Admin doctor photo upload")
        ...
        res_invalid_spec = await client.post(
            "/api/admin/doctors", json=invalid_doc, headers=headers_admin
        )
        assert (
            res_invalid_spec.status_code == 400
        ), "Expected 400 for unapproved specialty"
        ...
        res_create_doc = await client.post(
            "/api/admin/doctors", json=doc_payload, headers=headers_admin
        )
        assert res_create_doc.status_code in (
            200,
            201,
        ), f"Doctor creation failed: {res_create_doc.text}"
        ...
        res_initial_slots = await client.get(
            f"/api/doctors/{doctor_id}/slots?date={(date.today() + timedelta(days=1)).isoformat()}"
        )
        assert res_initial_slots.status_code == 200
        assert (
            len(res_initial_slots.json().get("slots", [])) == 0
        ), "Doctor must start with ZERO initial slots!"
        print("Verified newly created doctor starts with zero initial slots")

        print("[TEST 4 PASSED] Admin doctor creation and specialty validation")
        ...
        res_doc_as_pat = await client.post(
            "/api/auth/login",
            json={"identifier": doctor_email, "password": doctor_password},
        )
        assert (
            res_doc_as_pat.status_code == 403
        ), f"Expected 403 for doctor at patient login, got {res_doc_as_pat.status_code}"
        print("Doctor rejected from patient login with HTTP 403")

        # Admin attempts patient login -> must be rejected with 403
        res_admin_as_pat = await client.post(
            "/api/auth/login",
            json={"identifier": "admin@telemed.com", "password": "admin"},
        )
        assert (
            res_admin_as_pat.status_code == 403
        ), f"Expected 403 for admin at patient login, got {res_admin_as_pat.status_code}"
        print("Admin rejected from patient login with HTTP 403")

        # Doctor logs in via admin-login -> succeeds
        res_doc_login = await client.post(
            "/api/auth/admin-login",
            json={"identifier": doctor_email, "password": doctor_password},
        )
        assert (
            res_doc_login.status_code == 200
        ), f"Doctor direct login failed: {res_doc_login.text}"
        ...
        print("[TEST 5 PASSED] Strict role separation guards")
        ...
        assert res_mismatch.status_code in (
            400,
            422,
        ), f"Expected 400 or 422 for password mismatch, got {res_mismatch.status_code}"
        ...
        assert (
            "dev_otp" not in reg_json
        ), "dev_otp must NOT be leaked in registration response!"
        ...
        assert res_ver.status_code in (
            200,
            201,
        ), f"OTP verification failed: {res_ver.text}"
        ...
        res_pat_as_admin = await client.post(
            "/api/auth/admin-login",
            json={"identifier": patient_email, "password": patient_password},
        )
        assert (
            res_pat_as_admin.status_code == 403
        ), f"Expected 403 for patient at admin login, got {res_pat_as_admin.status_code}"
        ...
        res_prof = await client.put(
            "/api/auth/me/profile",
            json={"city": "Mysuru", "blood_group": "AB+"},
            headers=headers_pat,
        )
        ...
        print(
            f"Patient profile updated: City={pat_prof['city']}, BloodGroup={pat_prof['blood_group']}"
        )

        print("[TEST 7 PASSED] Patient profile update")
        ...
        res_create_slots = await client.post(
            "/api/doctors/me/slots", json=slot_payload_future, headers=headers_doc
        )
        assert (
            res_create_slots.status_code == 200
        ), f"Slot creation failed: {res_create_slots.text}"
        future_slots = (
            res_create_slots.json()
            if isinstance(res_create_slots.json(), list)
            else res_create_slots.json().get("slots", [])
        )
        ...
        # Slot batch for live video consultation
        # Create the slot 10 minutes in the future so the booking request
        # cannot become a past-time booking while the test is running.
        now_time = datetime.now()
        slot_today_start = (
            (now_time + timedelta(minutes=10)).time().replace(second=0, microsecond=0)
        )
        slot_today_end_dt = datetime.combine(
            now_time.date(), slot_today_start
        ...
        res_today_slots = await client.post(
            "/api/doctors/me/slots", json=slot_payload_today, headers=headers_doc
        )
        assert (
            res_today_slots.status_code == 200
        ), f"Today slot creation failed: {res_today_slots.text}"
        today_slots = (
            res_today_slots.json()
            if isinstance(res_today_slots.json(), list)
            else res_today_slots.json().get("slots", [])
        )
        print(
            f"Doctor created active consultation slots for today: {[s['start_time'] for s in today_slots]}"
        )
        ...
        res_del_slot = await client.delete(
            f"/api/doctors/me/slots/{slot_to_delete}", headers=headers_doc
        )
        assert res_del_slot.status_code == 200
        print(f"Doctor successfully deleted available slot #{slot_to_delete}")

        print("[TEST 8 PASSED] Doctor dynamic slot management")

        # 9. Patient Books Future Appointment with Slot ID
        print("\n[TEST 9] Testing Patient Booking with Dynamic Slot ID...")
        res_pat_slots = await client.get(
            f"/api/doctors/{doctor_id}/slots?date={future_date_str}",
            headers=headers_pat,
        )
        ...
        assert (
            len(avail_slots) == 3
        ), f"Expected 3 remaining slots after delete, got {len(avail_slots)}"
        ...
        res_future_book = await client.post(
            "/api/appointments", json=book_future, headers=headers_pat
        )
        assert res_future_book.status_code in (
            200,
            201,
        ), f"Booking failed: {res_future_book.text}"
        future_appt = res_future_book.json()
        print(
            f"Patient booked appointment #{future_appt['appointment_number']} (Linked slot {chosen_future_slot['id']})"
        )

        # Double booking prevention (same slot_id)
        res_double_book = await client.post(
            "/api/appointments", json=book_future, headers=headers_pat
        )
        assert res_double_book.status_code in (
            400,
            409,
        ), f"Expected conflict for already booked slot, got {res_double_book.status_code}"
        print("Double-booking of same slot strictly rejected")

        # Time window guard: starting future consultation rejected
        res_early_start = await client.post(
            f"/api/consultations/start/{future_appt['id']}", headers=headers_doc
        )
        assert res_early_start.status_code == 403
        print("Time window guard rejected starting future consultation with HTTP 403")

        print("[TEST 9 PASSED] Patient booking and double-booking prevention")

        # 10. Patient Books Active Consultation Slot
        print(
            "\n[TEST 10] Patient Books Active Slot & Doctor Starts Video Consultation..."
        )
        ...
        res_active_book = await client.post(
            "/api/appointments", json=book_active, headers=headers_pat
        )
        assert res_active_book.status_code in (
            200,
            201,
        ), f"Active booking failed: {res_active_book.text}"
        active_appt = res_active_book.json()
        active_appt_id = active_appt["id"]
        print(
            f"Active appointment #{active_appt['appointment_number']} booked for active consultation"
        )

        # Doctor starts the consultation
        res_start = await client.post(
            f"/api/consultations/start/{active_appt_id}", headers=headers_doc
        )
        assert res_start.status_code in (
            200,
            201,
        ), f"Consultation start failed: {res_start.text}"
        consultation = res_start.json()
        room_id = consultation["room_id"]
        consultation_id = consultation["id"]
        print(
            f"Doctor started consultation! Room ID: {room_id} (ID: {consultation_id})"
        )

        # Patient joins consultation
        res_join = await client.post(
            f"/api/consultations/{room_id}/join", headers=headers_pat
        )
        assert res_join.status_code == 200
        print("Patient joined consultation record")

        print("[TEST 10 PASSED] Active consultation and video consultation flow")
        ...
        async with websockets.connect(ws_doc_url) as ws_doc:
            ...
            async with websockets.connect(ws_pat_url) as ws_pat:
                ...
                recv_offer = json.loads(
                    await asyncio.wait_for(ws_pat.recv(), timeout=5.0)
                )
                ...
                recv_answer = json.loads(
                    await asyncio.wait_for(ws_doc.recv(), timeout=5.0)
                )
                ...
                await ws_pat.send(
                    json.dumps(
                        {"type": "chat-message", "text": "Testing prohibited chat"}
                    )
                )
                err_frame = json.loads(
                    await asyncio.wait_for(ws_pat.recv(), timeout=5.0)
                )
                assert err_frame["type"] == "error"
                print(
                    "WebRTC Signaling passed & chat frame was rejected with error frame"
                )
        ...
        res_end = await client.post(
            f"/api/consultations/{room_id}/end",
            json={"clinical_notes": "Vitals stable, normal ECG."},
            headers=headers_doc,
        )
        assert res_end.status_code == 200
        print("Consultation session ended successfully")

        print("[TEST 11 PASSED] WebRTC signaling and chat prohibition")
        ...
        res_rx = await client.post(
            "/api/prescriptions", json=rx_payload, headers=headers_doc
        )
        ...
        res_duplicate_rx = await client.post(
            "/api/prescriptions", json=rx_payload, headers=headers_doc
        )
        assert res_duplicate_rx.status_code in (
            400,
            409,
        ), f"Expected conflict for duplicate prescription, got {res_duplicate_rx.status_code}"
        print("Duplicate prescription for same consultation strictly rejected")

        print(
            "[TEST 12 PASSED] Digital prescription generation and duplicate prevention"
        )
        ...
        res_doc_patients = await client.get(
            "/api/doctors/me/patients", headers=headers_doc
        )
        assert res_doc_patients.status_code == 200
        assigned_patients = res_doc_patients.json()
        assert any(
            p["id"] == patient_profile_id for p in assigned_patients
        ), "Patient must appear in doctor assigned patients"
        print(f"Doctor retrieved {len(assigned_patients)} assigned patients")

        # Doctor views patient history
        res_history = await client.get(
            f"/api/doctors/me/patients/{patient_profile_id}/history",
            headers=headers_doc,
        )
        assert (
            res_history.status_code == 200
        ), f"History access failed: {res_history.text}"
        ...
        print(
            "Doctor accessed patient longitudinal history (demographics, previous consultations, and prescriptions)"
        )

        # Doctor attempts history access for an unauthorized random patient ID
        res_unauth_hist = await client.get(
            "/api/doctors/me/patients/999999/history", headers=headers_doc
        )
        assert res_unauth_hist.status_code in (
            403,
            404,
        ), f"Expected 403/404 for unassigned patient, got {res_unauth_hist.status_code}"
        print("History access denied for unassigned patient")

        print("[TEST 13 PASSED] Doctor patient access and clinical history")
        ...
        res_temp_doc = await client.post(
            "/api/admin/doctors",
            json={
                "email": temp_email,
        ...
        # Delete doctor
        res_del_doc = await client.delete(
            f"/api/admin/doctors/{temp_doc_id}", headers=headers_admin
        )
        assert (
            res_del_doc.status_code == 200
        ), f"Doctor deletion failed: {res_del_doc.text}"
        ...
        print("[TEST 14 PASSED] Admin doctor deletion")
